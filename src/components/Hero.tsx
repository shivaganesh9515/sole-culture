...
const hotspots = [
  { num: 1, label: 'Breathable Mesh Upper', left: '30%', top: '40%' },
  { num: 2, label: 'Customizable Lace Loops', left: '60%', top: '25%' },
  { num: 3, label: 'Impact Absorbing Sole', left: '80%', top: '60%' },
  { num: 4, label: 'Arch Support Technology', left: '45%', top: '80%' },
];

export default function Hero() {
  return (
    <section className="relative border-b-2 border-brand-black overflow-hidden">
      <div className="absolute inset-0 z-0 opacity-50 pointer-events-none" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1594492215849-f554425dd943?q=80&w=2000&auto=format&fit=crop')", backgroundSize: 'cover', backgroundPosition: 'center' }} />
      ...
      <div className="max-w-[1920px] mx-auto grid grid-cols-1 lg:grid-cols-2 min-h-[600px] relative z-10">
        ...
        <div className="p-8 md:p-16 flex flex-col justify-center border-r-2 border-brand-black bg-white/80 backdrop-blur-sm lg:bg-transparent lg:backdrop-blur-none">
          <h1 className="text-5xl md:text-7xl font-black leading-[0.9] mb-12 uppercase tracking-tighter">
            Create your first <br /> customizable <span className="text-stroke-black text-transparent bg-clip-text bg-brand-black">Sneakers</span>
          </h1>
          ...
          <form className="space-y-6 max-w-md relative z-20">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <input type="text" placeholder="YOUR NAME" className="w-full border-2 border-brand-black p-3 bg-white font-mono text-sm focus:outline-none focus:ring-4 focus:ring-brand-lime" />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-wider">Email</label>
                <input type="email" placeholder="YOUR EMAIL" className="w-full border-2 border-brand-black p-3 bg-white font-mono text-sm focus:outline-none focus:ring-4 focus:ring-brand-lime" />
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-wider">Tell us about your wishes</label>
              <textarea rows={4} placeholder="YOUR WISHES" className="w-full border-2 border-brand-black p-3 bg-white font-mono text-sm focus:outline-none focus:ring-4 focus:ring-brand-lime resize-none" />
            </div>
            <button type="button" className="w-full bg-brand-black text-brand-white py-4 font-bold uppercase tracking-widest hover:bg-brand-lime hover:text-brand-black transition-all border-2 border-brand-black shadow-brutal hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-brutal-sm active:translate-x-[4px] active:translate-y-[4px] active:shadow-none">
              Send Message
            </button>
          </form>
        </div>
        ...
        <div className="relative flex items-center justify-center min-h-[400px] bg-brand-sky/30 overflow-hidden">
          ...
          <div className="relative w-full max-w-lg mx-auto transform hover:scale-105 transition-transform duration-500 p-6">
            <img src="https://images.unsplash.com/photo-1549298916-b41d501d3772?q=80&w=1000&auto=format&fit=crop" alt="White Sneaker" className="w-full h-auto drop-shadow-2xl relative z-10 rotate-[-15deg] block" />

            {hotspots.map((h) => (
              <div key={h.num} className="absolute z-20 group" style={{ left: h.left, top: h.top }}>
          ...
          <div className="absolute top-10 left-10 bg-white border-2 border-brand-black p-3 max-w-[200px] shadow-brutal rotate-[-2deg] hidden md:block">
            <p className="text-xs font-mono">Create™: the lightweight foam resin that isn&apos;t rubber, isn&apos;t plastic.</p>
          </div>
          <div className="absolute bottom-20 right-10 bg-white border-2 border-brand-black p-3 max-w-[200px] shadow-brutal rotate-[3deg] hidden md:block">
            <p className="text-xs font-mono">Heel strap keeps this thing on your feet.</p>
          </div>
        </div>
      </div>
    </section>
